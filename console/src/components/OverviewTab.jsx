...
export default function OverviewTab({ derived, receiver, token, mode, busy }) {
  const hasStatus = typeof derived?.status === "string"
  if (!hasStatus) {
    return (
      <div className="tab-content">
        ...
        <div className="card">
          <div className="empty-row">{busy ? "Syncing latest status..." : "No status data loaded yet."}</div>
        </div>
      </div>
    )
  }
  ...
  const sourceAgreement = derived?.sourceMismatch ? "DEGRADED" : "HEALTHY"
    ...
    { label: "Data freshness", value: worstAge ? `worst ${formatAge(worstAge)}` : "--" },
    {
      ...
      value: (
        <span style={{ display: "inline-flex", alignItems: "center", gap: "10px" }}>
          <StatusPill status={sourceAgreement} />
          <span>{derived?.sourceMismatch ? "mismatch" : "aligned"}</span>
        </span>
      ),
    },
